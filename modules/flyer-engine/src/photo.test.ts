...
vi.mock("@club-os/core/database/supabase", () => ({
  supabaseAdmin: { from: vi.fn(), storage: { from: vi.fn() } },
}));
vi.mock("@club-os/core/search/socialcrawl", () => ({
  searchInstagramCandidates: vi.fn(),
  resolveOfficialAccount: vi.fn(),
  getInstagramPostPhotos: vi.fn(),
  getInstagramProfilePhoto: vi.fn(),
}));
vi.mock("@club-os/core/search/googleImageSearch", () => ({ findArtistPhotosViaGoogle: vi.fn() }));
vi.mock("@club-os/core/search/braveImageSearch", () => ({ findArtistPhotosViaBrave: vi.fn() }));
vi.mock("@club-os/core/search/deezerImageSearch", () => ({ findArtistPhotoViaDeezer: vi.fn() }));
...
import { searchInstagramCandidates, resolveOfficialAccount, getInstagramPostPhotos } from "@club-os/core/search/socialcrawl";
